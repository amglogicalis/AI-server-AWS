...
# Set high DPI awareness on Windows for crisp fonts and layout resolution
try:
    import ctypes
...
if len(sys.argv) > 1:
    ...
    if arg_path and not arg_path.startswith("-"):
        CRED_PATH = os.path.abspath(arg_path)
...
def validate_credentials_cli(profile):
    ...
    try:
        # Pass the credentials file path via environment variable
        env = os.environ.copy()
...
class AwsConfigureGuiApp:
    ...
    def setup_ui(self):
        ...
        self.entry_profile = self.create_modern_entry(row_profile, side=tk.LEFT, fill=tk.X, expand=True)
        ...
        self.entry_ak = self.create_modern_entry(main_frame, fill=tk.X, pady=(0, 15))
        ...
        self.entry_sk = self.create_modern_entry(row_sk, show="•", side=tk.LEFT, fill=tk.X, expand=True)
    ...
    # -------------------------------------------------------
    def create_modern_entry(self, parent, show=None, **pack_kwargs):
