...
IS_WSL=0
if grep -qEi 'Microsoft|Microsoft-Standard' /proc/version &>/dev/null; then
    IS_WSL=1
fi

# Check if GUI is supported in the current environment (Native Linux GUI)
GUI_AVAILABLE=0
...
if [ $GUI_AVAILABLE -eq 1 ]; then
    ...
    python3 "$SCRIPT_DIR/awsconfigure_gui.py" "$CRED_PATH" "$@"
    exit $?
elif [ $IS_WSL -eq 1 ] && command -v python.exe &>/dev/null; then
    WIN_SCRIPT_PATH=$(wslpath -w "$SCRIPT_DIR/awsconfigure_gui.py")
    WIN_CRED_PATH=$(wslpath -w "$CRED_PATH")
    python.exe "$WIN_SCRIPT_PATH" "$WIN_CRED_PATH" "$@"
    exit $?
fi

echo "Nota: Interfaz gráfica no disponible (\$DISPLAY vacío o falta el paquete python3-tk y python.exe de Windows)."
